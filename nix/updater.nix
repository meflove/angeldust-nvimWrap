{
  ...
  ourPackages = self.packages.${pkgs.stdenv.hostPlatform.system} or {};
...
in
  ...
  ''
    ...
    for name in ${toString packagesToUpdate}; do
      ...
      cmd="cd $PWD && $nix_update_cmd --flake packages.${pkgs.stdenv.hostPlatform.system}.$name --version=branch"
